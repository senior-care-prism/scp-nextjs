...
const RESOURCE_CONTENTFUL_GRAPHQL_FIELDS = `
sys {
  id
}
`;
...
${RESOURCE_CONTENTFUL_GRAPHQL_FIELDS}
title
slug
publishedDate
shortDescription
...
export async function getResourceEntries({ pageNum = 1, limit = 9, search = '' } = {}) {
  const offset = (pageNum - 1) * limit;
  const searchFilter = search !== ''
    ? `where: { OR: [ 
    ...
    : '';
  const resourceEntries = await fetchGraphQL(
    `query {
