...
export async function getNewsEntries({ pageNum = 1, limit = 6, search = '' } = {}) {
  ...
  const searchFilter = search !== ''
    ? `where: { OR: [ 
      { headline_contains: "${decodeURIComponent(search)}" },
      { content_contains: "${decodeURIComponent(search)}" } ]}, `
    : '';
...
export async function getNewsMaxPage({ limitPerPage = 6, search } = {}) {
  ...
  return totalEntries ? { total: totalEntries, maxPage: Math.ceil(totalEntries / limitPerPage) } : { total: 0, maxPage: 0 };
}
...
export async function getResourceEntries({ pageNum = 1, limit = 6, category = '', subject = '', searchTerm = '' } = {}) {
  ...
  const categoryFilter = category === '' ? '' : `{ category_contains_some: ${decodeURIComponent(JSON.stringify(selectedCategories))} }`;
  const subjectFilter = subject === '' ? '' : `{subject_contains_some: ${decodeURIComponent(JSON.stringify(selectedSubjects))} }`;
  const searchTermFilter = searchTerm === '' ? '' : `{ OR: [ 
                                                          { title_contains: "${decodeURIComponent(searchTerm)}" },
                                                          { shortDescription_contains: "${decodeURIComponent(searchTerm)}" },
                                                          { description_contains: "${decodeURIComponent(searchTerm)}" } 
                                                        ]
                                                      }`;
...
export async function getAllResourceCategories(subjects = '') {
  ...
  const subjectFilter = subjects === '' ? '' : `{ subject_contains_some: ${decodeURIComponent(JSON.stringify(selectedSubjects))} }`;
  const entries = await fetchGraphQL(
    `query {
...
export async function getAllResourceSubjects(categories = '') {
  ...
  const categoryFilter = categories === '' ? '' : `{ category_contains_some: ${decodeURIComponent(JSON.stringify(selectedCategories))} }`;
  const entries = await fetchGraphQL(
    `query {
...
export async function getQuotes({ limit = 6 } = {}) {
  const entry = await fetchGraphQL(
    `query {
      quoteCollection( limit: ${limit}, order: id_ASC) {
        items {
...
export async function getTeamMembers({ limit = 6 } = {}) {
  const entry = await fetchGraphQL(
    `query {
      teamMemberCollection( limit: ${limit}, order: id_ASC) {
        items {
