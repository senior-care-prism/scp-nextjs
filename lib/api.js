...
export async function getResourceEntries({ pageNum = 1, limit = 9, category = '', subject = '', searchTerm = '' } = {}) {
  const offset = (pageNum - 1) * limit;
  const categoryFilter = category === '' ? '' : `{ category_contains_all: "${category}" }`;
  const subjectFilter = subject === '' ? '' : `{subject_contains: "${subject}" }`;
...
export async function getResourceMaxPage({ limitPerPage = 9, search } = {}) {
  const searchFilter = search !== ''
    ? `where: { 
      AND: [
        {
          OR: [
            ...
            { shortDescription_contains: "${search}" },
...
export async function getAllResourceCategories() {
  const entries = await fetchGraphQL(
    `query {
      resourceCollection(where: { category_exists: true }) {
        items {
          category
        }
      }
...
export async function getAllResourceSubjects() {
  const entries = await fetchGraphQL(
    `query {
      resourceCollection(where: { subject_exists: true }) {
        items {
          subject
        }
      }
